...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN:
		hFindWindow = FindWindow(NULL, (LPCSTR)L"MsgClient");
		if (hFindWindow != NULL)
			SendMessage(hFindWindow, WM_IPC_MSG, (WPARAM)65, (LPARAM)0);
		else
			MessageBox(hWnd, (LPCSTR)L"클라이언트를 찾을 수 없습니다.", (LPCSTR)L"오류", MB_OK);
		return 0;
	case WM_PAINT:
